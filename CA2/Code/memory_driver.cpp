...
int main (int argc, char* argv[]) // the program runs like this: ./program <filename> <mode>
{
	...
	double L1_miss_rate, L2_miss_rate, Victim_miss_rate;
	//compute the stats here:
	L1_miss_rate = myCache.get_L1_miss_rate();
	L2_miss_rate = myCache.get_L2_miss_rate();
	Victim_miss_rate = myCache.get_Victim_miss_rate();

	double L2_access_time, Victim_access_time, AAT;
	L2_access_time = 8 + L2_miss_rate * 100;
	Victim_access_time = 1 + Victim_miss_rate * L2_access_time;
	AAT = 1 + L1_miss_rate * Victim_access_time;
